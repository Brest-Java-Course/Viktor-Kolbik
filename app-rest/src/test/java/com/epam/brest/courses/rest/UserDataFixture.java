...
public class UserDataFixture {
    ...
    public static User getExistingUser(String login){
        return new User(3L, login, SOME_NAME);
    }

    public static List<User> getExistingUsers(){
        List<User> users = new ArrayList<User>();

        users.add(new User(SOME_ID + 1, SOME_LOGIN + 1, SOME_NAME + 1));
        users.add(new User(SOME_ID + 2, SOME_LOGIN + 2, SOME_NAME + 2));
        users.add(new User(SOME_ID + 3, SOME_LOGIN + 3, SOME_NAME + 3));

        return users;
    }

    public static User getNewUser(){
        return new User(null, "some login", "some name");
    }
